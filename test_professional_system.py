...
from sentiment_bot.cli_unified import (
    _fetch_articles,
    _deduplicate_articles,
    _filter_by_freshness,
)
...
async def test_professional_pipeline():
    ...
    plan = planner.plan_selection(region=region, topics=[topic])
    ...
    console.print(
        f"Fresh: {len(fresh_articles)}/{len(unique_articles)} (Rate: {freshness_rate:.1%})"
    )

    # Step 5: Clustering
    console.print("\n[bold]🎯 Step 5: Article Clustering[/bold]")
    clusterer = DocumentClusterer()
    clustered_articles = clusterer.cluster_articles(
        fresh_articles[:20]
    )  # Limit for testing
    unique_clusters = len(set(a["cluster_id"] for a in clustered_articles))
    console.print(
        f"Found {unique_clusters} unique clusters from {len(clustered_articles)} articles"
    )
    ...
    with Progress() as progress:
        ...
        for article in clustered_articles:
            ...
            if text or title:
                ...
                aspects = aspect_extractor.extract_aspects(
                    text, {"region": region, "topic": topic}
                )
    ...
    console.print(
        f"Analyzed {len(analyzed_articles)} articles, {abstained_count} abstained"
    )
    ...
    sentiment_scores = [
        a["sentiment_score"] for a in analyzed_articles if not a.get("abstained")
    ]
    avg_sentiment = (
        sum(sentiment_scores) / len(sentiment_scores) if sentiment_scores else 0
    )
    avg_confidence = (
        sum(a["confidence"] for a in analyzed_articles) / len(analyzed_articles)
    ...
    results = {
        ...
        "sentiment": {
            "positive": sum(
                1 for a in analyzed_articles if a["sentiment_label"] == "positive"
            ),
            "negative": sum(
                1 for a in analyzed_articles if a["sentiment_label"] == "negative"
            ),
            "neutral": sum(
                1 for a in analyzed_articles if a["sentiment_label"] == "neutral"
            ),
        },
    }
    ...
    performance = {
        ...
        "freshness_rate": freshness_rate,
        "fresh_words": sum(
            len(a.get("content", a.get("description", "")).split())
