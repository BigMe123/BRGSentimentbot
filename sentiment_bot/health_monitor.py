...
class HealthMonitor:
    ...
    def record_relevance(
        self, domain: str, relevance_score: float, dropped: bool = False
    ):
    ...
    def auto_tune_sources(self, dry_run: bool = False) -> Dict[str, List[str]]:
        ...
        """
        actions = {"promoted": [], "demoted": [], "parked": []}
        ...
        for domain, metrics in self.metrics.items():
            ...
            if health_score < self.parking_threshold:
                ...
                if metrics.fetch_failures >= 10 and metrics.success_rate < 0.1:
                    logger.info(
                        f"Parking dead source {domain} (health: {health_score:.2f})"
                    )
                    if not dry_run:
    ...
    def get_run_metrics(self) -> Dict[str, Any]:
        ...
        # Calculate averages
        avg_latency = (
            statistics.mean(m.avg_latency for m in self.metrics.values() if m.latencies)
            if any(m.latencies for m in self.metrics.values())
            else 0
        )
        ...
        avg_relevance = (
            ...
            if any(m.relevance_scores for m in self.metrics.values())
            else 0
        )
        ...
        return {
            ...
            "freshness_rate": (
                fresh_articles / total_articles if total_articles > 0 else 0
            ),
    ...
    def export_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "source_metrics": {
                domain: self.get_source_report(domain) for domain in self.metrics.keys()
            },
        }
