...
class MetricsCollector:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if self.fetch_latencies:
            ...
            metrics["p50_fetch_latency_ms"] = sorted_latencies[
                len(sorted_latencies) // 2
            ]
            metrics["p95_fetch_latency_ms"] = sorted_latencies[
                int(len(sorted_latencies) * 0.95)
            ]
            metrics["p99_fetch_latency_ms"] = sorted_latencies[
    ...
    def check_alerts(self) -> List[Alert]:
        ...
        alert_conditions = [
            ...
            (
                "error",
                "fraction_published_24h",
                "<",
                0.60,
                "Less than 60% of articles are fresh (<24h)",
            ),
            ("error", "timeout_rate", ">", 0.15, "Timeout rate exceeds 15%"),
            (
                "error",
                "headless_usage_rate",
                ">",
