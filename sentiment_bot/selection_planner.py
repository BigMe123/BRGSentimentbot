...
class SelectionPlan:
    ...
    def get_diversity_score(self) -> float:
        ...
        if len(self.editorial_families) >= self.quotas.min_editorial_families:
            ...
            score += 0.33 * (len(self.editorial_families) / self.quotas.min_editorial_families)
...
class SelectionPlanner:
    ...
    def plan_selection(self,
                       ...
                       strict: bool = False,
                       expand: bool = False,
                       quotas: Optional[SelectionQuotas] = None) -> SelectionPlan:
        """
        ...
        if other_topic and not topics:
            ...
            if not topics:
                logger.warning(f"No topics matched for '{other_topic}', will trigger discovery")
        ...
        logger.info(f"Selected {len(plan.sources)} sources in {plan.selection_time_ms:.1f}ms, "
    ...
    def _get_candidates(self, 
        ...
        if region and topics:
            ...
            candidates = self.catalog.get_sources_by_criteria(
                region=region,
                topics=topics,
                min_priority=0.3 if not strict else 0.5
            )
        elif region:
            # Region only
    ...
    def _score_sources(self, 
                      sources: List[SourceRecord],
                      region: Optional[str],
                      topics: Optional[List[str]]) -> List[Tuple[SourceRecord, float]]:
    ...
    def _apply_quotas(self, 
                     scored: List[Tuple[SourceRecord, float]],
                     quotas: SelectionQuotas) -> List[SourceRecord]:
        """Apply quotas and diversity requirements."""
        selected = []
        ...
        for source, score in scored:
            ...
            if (len(selected) >= quotas.min_sources and
                len(editorial_families) >= quotas.min_editorial_families and
                len(languages) >= quotas.min_languages):
                # We've met quotas, but continue to max for better coverage
                pass
        
        logger.info(f"Selected {len(selected)} sources with {len(editorial_families)} "
    ...
    def _calculate_diversity(self, plan: SelectionPlan):
        ...
        for source in plan.sources:
            # Editorial family
            if 'wire' in source.notes.lower() or 'agency' in source.name.lower():
                plan.editorial_families['wire'] = plan.editorial_families.get('wire', 0) + 1
            elif 'think' in source.notes.lower():
                plan.editorial_families['think_tank'] = plan.editorial_families.get('think_tank', 0) + 1
            else:
                plan.editorial_families['publication'] = plan.editorial_families.get('publication', 0) + 1
            
            # Languages
            for lang in source.languages:
                plan.language_distribution[lang] = plan.language_distribution.get(lang, 0) + 1
            
            # Regions
            plan.region_distribution[source.region] = plan.region_distribution.get(source.region, 0) + 1
    ...
    def _allocate_time_budget(self, plan: SelectionPlan):
        ...
        for source in plan.sources:
            ...
            allocation = (source_score / total_score) * total_budget if total_score > 0 else 0
