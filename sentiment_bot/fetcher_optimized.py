...
class OptimizedFetcher:
    ...
    async def fetch_article_content(self, article: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    async def feed_worker(self):
        ...
        while not self.stop_requested:
            try:
                feed_url = await asyncio.wait_for(
                    self.feed_queue.get(), 
                    timeout=1.0
                )
    ...
    async def process_feeds(self, feed_urls: List[str]) -> FetchResult:
        ...
        while True:
            ...
            if (self.feed_queue.empty() and 
                self.article_queue.empty() and 
