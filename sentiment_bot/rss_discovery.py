...
class RSSDiscovery:
    ...
    def __init__(self):
        self.common_paths = CONFIG.get("discovery", {}).get(
            "common_paths",
            [
                "/rss",
                "/feed",
    ...
    async def resolve_feeds(
        self, domain: str, region: str = None, topic: str = None
    ) -> List[str]:
        """
    ...
    def _parse_html_for_feeds(self, html: str, base_url: str) -> List[str]:
        ...
        try:
            ...
            for anchor in rss_anchors[:5]:  # Limit to prevent too many
                ...
                if href and any(
                    x in href.lower() for x in ["rss", "feed", "atom", ".xml"]
                ):
                    feed_url = urljoin(base_url, href)
                    feeds.append(feed_url)
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    if any(
                        ...
                        for marker in ["<rss", "<feed", "<channel", "<?xml"]
                    ):
                        return url
        except:
            pass
