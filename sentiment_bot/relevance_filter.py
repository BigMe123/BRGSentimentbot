...
class RelevanceScore:
    ...
    def should_keep(self) -> bool:
        ...
        return (self.region_score >= 0.3 or self.topic_score >= 0.3 or 
                (self.region_score >= 0.2 and self.topic_score >= 0.2))
...
class RelevanceFilter:
    ...
    def _build_region_patterns(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            'asia': {
                ...
                'cities': [
                    'beijing', 'shanghai', 'tokyo', 'seoul', 'jakarta', 'bangkok',
                    'manila', 'kuala lumpur', 'singapore', 'new delhi', 'mumbai',
                    'karachi', 'dhaka', 'colombo', 'kathmandu', 'yangon', 'taipei',
                    'hong kong', 'osaka', 'kyoto', 'busan', 'hanoi', 'ho chi minh'
                ],
                'regions': [
                    'asia', 'asian', 'asia-pacific', 'apac', 'asean', 'south asia',
                    'southeast asia', 'east asia', 'central asia', 'indo-pacific'
                ],
                'url_patterns': ['/asia/', '/asian/', '/apac/', '/asean/'],
            },
            'middle_east': {
                'countries': [
                    'israel', 'palestine', 'jordan', 'lebanon', 'syria', 'iraq',
                    'iran', 'saudi arabia', 'uae', 'kuwait', 'qatar', 'bahrain',
                    'oman', 'yemen', 'egypt', 'turkey', 'libya', 'tunisia', 'algeria',
                    'morocco'
                ],
                'cities': [
                    'jerusalem', 'tel aviv', 'gaza', 'beirut', 'damascus', 'baghdad',
                    'tehran', 'riyadh', 'dubai', 'abu dhabi', 'kuwait city', 'doha',
                    'cairo', 'istanbul', 'ankara', 'tripoli', 'tunis', 'algiers',
                    'rabat', 'amman', 'sana\'a'
                ],
                'regions': [
                    'middle east', 'mideast', 'mena', 'gulf', 'levant', 'gcc',
                    'arab', 'north africa', 'maghreb'
                ],
                'url_patterns': ['/middle-east/', '/mideast/', '/mena/', '/gulf/'],
            },
            'europe': {
                'countries': [
                    'germany', 'france', 'uk', 'united kingdom', 'britain', 'italy',
                    'spain', 'poland', 'ukraine', 'romania', 'netherlands', 'belgium',
                    'greece', 'portugal', 'czech', 'hungary', 'sweden', 'austria',
                    'belarus', 'switzerland', 'bulgaria', 'denmark', 'finland',
                    'slovakia', 'norway', 'ireland', 'croatia', 'moldova', 'bosnia',
                    'albania', 'lithuania', 'slovenia', 'latvia', 'estonia'
                ],
                'cities': [
                    'london', 'paris', 'berlin', 'madrid', 'rome', 'kiev', 'kyiv',
                    'bucharest', 'warsaw', 'budapest', 'vienna', 'prague', 'brussels',
                    'amsterdam', 'lisbon', 'athens', 'stockholm', 'copenhagen',
                    'helsinki', 'oslo', 'dublin', 'zurich', 'geneva', 'milan'
                ],
                'regions': [
                    'europe', 'european', 'eu', 'eurozone', 'schengen', 'balkans',
                    'scandinavia', 'nordic', 'eastern europe', 'western europe'
                ],
            ...
            'americas': {
                'countries': [
                    ...
                    'guatemala', 'cuba', 'haiti', 'dominican', 'honduras', 'nicaragua',
                    'el salvador', 'costa rica', 'panama', 'jamaica', 'trinidad'
                ],
                'cities': [
                    'washington', 'new york', 'los angeles', 'chicago', 'toronto',
                    'mexico city', 'sao paulo', 'rio', 'buenos aires', 'lima',
                    'bogota', 'caracas', 'santiago', 'montevideo', 'quito', 'havana',
                    'panama city', 'san jose', 'guatemala city'
                ],
                'regions': [
                    'americas', 'north america', 'south america', 'latin america',
                    'central america', 'caribbean', 'nafta', 'mercosur'
                ],
                'url_patterns': ['/americas/', '/us/', '/usa/', '/latin-america/'],
            },
            'africa': {
                'countries': [
                    'south africa', 'nigeria', 'egypt', 'ethiopia', 'kenya', 'uganda',
                    'algeria', 'sudan', 'morocco', 'angola', 'mozambique', 'ghana',
                    'madagascar', 'cameroon', 'ivory coast', 'niger', 'burkina faso',
                    'mali', 'malawi', 'zambia', 'zimbabwe', 'somalia', 'senegal',
                    'rwanda', 'tunisia', 'guinea', 'benin', 'burundi', 'libya'
                ],
                'cities': [
                    'cairo', 'lagos', 'kinshasa', 'johannesburg', 'cape town',
                    'nairobi', 'addis ababa', 'accra', 'algiers', 'casablanca',
                    'tunis', 'kampala', 'harare', 'lusaka', 'dakar', 'khartoum'
                ],
                'regions': [
                    'africa', 'african', 'sub-saharan', 'north africa', 'west africa',
                    'east africa', 'southern africa', 'central africa', 'sahel',
                    'maghreb', 'horn of africa'
                ],
    ...
    def _build_topic_lexicons(self) -> Dict[str, List[str]]:
        ...
        return {
            'elections': [
                'election', 'vote', 'voting', 'ballot', 'poll', 'polls', 'campaign',
                'candidate', 'electoral', 'democracy', 'democratic', 'referendum',
                'primary', 'caucus', 'constituency', 'voter', 'turnout', 'incumbent',
                'opposition', 'ruling party', 'parliament', 'congress', 'senate',
                'assembly', 'election commission', 'electoral college', 'swing state',
                'exit poll', 'postal vote', 'absentee', 'recount', 'runoff'
            ],
            'security': [
                'security', 'military', 'defense', 'defence', 'army', 'navy', 'air force',
                'troops', 'forces', 'conflict', 'war', 'attack', 'terrorism', 'terrorist',
                'extremist', 'militant', 'insurgent', 'peacekeeping', 'nato', 'alliance',
                'missile', 'nuclear', 'weapons', 'arms', 'cyber', 'threat', 'intelligence',
                'surveillance', 'border', 'checkpoint'
            ],
            'economy': [
                'economy', 'economic', 'gdp', 'inflation', 'unemployment', 'market',
                'markets', 'stock', 'bond', 'currency', 'forex', 'trade', 'export',
                'import', 'tariff', 'deficit', 'surplus', 'budget', 'fiscal', 'monetary',
                'central bank', 'interest rate', 'recession', 'growth', 'investment',
                'finance', 'financial', 'banking', 'imf', 'world bank', 'debt'
            ],
            'politics': [
                'politics', 'political', 'government', 'president', 'prime minister',
                'minister', 'cabinet', 'parliament', 'congress', 'senate', 'policy',
                'legislation', 'law', 'bill', 'reform', 'opposition', 'ruling',
                'coalition', 'party', 'diplomatic', 'embassy', 'ambassador', 'summit',
                'bilateral', 'multilateral', 'treaty', 'agreement', 'sanctions'
            ],
            'energy': [
                'energy', 'oil', 'gas', 'petroleum', 'opec', 'crude', 'barrel', 'pipeline',
                'refinery', 'drilling', 'fracking', 'renewable', 'solar', 'wind', 'nuclear',
                'electricity', 'power', 'grid', 'blackout', 'lng', 'natural gas', 'coal',
                'mining', 'uranium', 'hydroelectric', 'battery', 'lithium', 'hydrogen'
            ],
            'climate': [
                'climate', 'climate change', 'global warming', 'carbon', 'emissions',
                'greenhouse', 'paris agreement', 'cop', 'ipcc', 'temperature', 'drought',
                'flood', 'hurricane', 'typhoon', 'cyclone', 'wildfire', 'heatwave',
                'melting', 'glacier', 'sea level', 'renewable', 'sustainability',
                'environmental', 'pollution', 'deforestation', 'biodiversity'
            ],
            'tech': [
                'technology', 'tech', 'digital', 'ai', 'artificial intelligence',
                'machine learning', 'algorithm', 'data', 'cyber', 'internet', 'online',
                'software', 'hardware', 'chip', 'semiconductor', 'smartphone', '5g',
                'blockchain', 'crypto', 'bitcoin', 'startup', 'innovation', 'research',
                'development', 'silicon valley', 'tech giant', 'platform', 'app'
            ]
        }
    ...
    def _build_dateline_patterns(self) -> List[re.Pattern]:
        ...
        return [
            ...
            re.compile(r'^([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*),?\s+[A-Z][a-z]+\s+\d+', re.MULTILINE),  # "Cairo, August 9"
    ...
    def _score_region(self, text: str, url: str, target_region: str) -> Tuple[float, List[str]]:
    ...
    def _score_topic(self, text: str, url: str, target_topic: str) -> Tuple[float, List[str]]:
    ...
    def _entity_matches_region(self, entity: str, target_region: str) -> bool:
        ...
        return (
            entity_lower in patterns['countries'] or
            entity_lower in patterns['cities'] or
            any(region in entity_lower for region in patterns['regions'])
        )