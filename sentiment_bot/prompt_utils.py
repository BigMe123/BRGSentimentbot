...
def safe_prompt(
    ...
    try:
        return Prompt.ask(message, default=default, choices=choices, password=password)
...
def safe_choice(message: str, choices: List[str], default: Optional[str] = None) -> str:
    """
...
def safe_multi_select(
    message: str, choices: List[str], defaults: Optional[List[str]] = None
) -> List[str]:
    """
...
def handle_interactive_menu(
    stats: dict, results: dict, articles: list, default_view: str = "Summary"
) -> None:
    """
