...
def run(
    ...
    other: Optional[str] = typer.Option(None, "--other", "-o",
        help="Free-text topic for obscure subjects"),
    ...
    min_sources: int = typer.Option(None, "--min-sources",
        help="Minimum number of sources (default from config)"),
    target_words: int = typer.Option(10000, "--target-words",
        help="Target fresh words to collect"),
    discover: bool = typer.Option(None, "--discover", "-d",
        help="Enable RSS discovery (default from config)"),
    max_age_hours: int = typer.Option(None, "--max-age-hours",
...
async def _run_async(
    ...
    if len(resolved_sources) < min_sources or len(editorial_families) < CONFIG.get("selection", {}).get("require_editorial_families", 4):
        ...
        for source in global_sources:
            ...
            if feeds:
                resolved_sources.append(SourceRecord(
    ...
    fresh_articles, stale_count, freshness_rate = _filter_by_freshness(articles, max_age_hours)
    
    console.print(f"[green]Fresh: {len(fresh_articles)}/{len(articles)} articles (Freshness Rate: {freshness_rate:.1%})[/green]")
    ...
    console.print(f"[green]Kept {len(filtered_articles)}/{len(fresh_articles)} fresh articles after relevance filter[/green]")
    ...
    fresh_words = sum(len(article.get('content', '').split()) for article in filtered_articles)
    ...
    _display_enhanced_results(results, {
        ...
        'fetch_success_rate': monitor.get_run_metrics().get('fetch_success_rate', 0),
...
async def _get_backfill_sources(needed: int, existing_families: Set[str], region: str = None, topic: str = None) -> List[Dict]:
...
async def _fetch_articles_with_retry(sources: List[SourceRecord], monitor) -> List[Dict]:
                ...
                if result and result.entries:
                    ...
                    for entry in result.entries[:10]:
                        ...
                        if article['published']:
                            try:
                                article['published_date'] = date_parser.parse(article['published'])
                            except:
...
def _display_enhanced_results(results: Dict, metrics: Dict):
    ...
    table.add_row("Sentiment", f"Pos: {results['sentiment']['positive']}, "
