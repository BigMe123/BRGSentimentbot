...
class OptimizedHTTPClient:
    ...
    def _record_failure(self, domain: str, reason: str):
        ...
        if cb['failures'] >= 3:
            ...
            logger.warning(f"Circuit breaker opened for {domain} after {cb['failures']} failures")
                        ...
                        if stream_chunks:
                            async for chunk in response.content.iter_chunked(65536):  # 64KB chunks
                                ...
                                if bytes_read > self.max_response_size:
                                    logger.warning(f"Response size exceeded {self.max_response_size} bytes for {url}")
                                    break
                        else:
    ...
    async def fetch_batch(self, urls: list[str]) -> list[Tuple[Optional[bytes], Dict[str, Any]]]:
