...
class SourceDiscovery:
    ...
    COMMON_RSS_PATHS = [
        ...
        '/atom.xml', '/index.xml', '/feed/rss', '/rss/feed',
    ...
    SITEMAP_PATHS = [
        ...
        '/sitemap_news.xml', '/sitemaps/sitemap.xml'
    ]
    ...
    async def discover_sources(self, 
                              ...
                              time_budget: float = 30.0) -> List[DiscoveryResult]:
        """
        ...
        async with aiohttp.ClientSession(timeout=timeout_config) as session:
            ...
            for url in seed_urls[:self.max_domains]:
                ...
                tasks.append(self._discover_from_url(
                    session, url, topic_keywords, region
                ))
        ...
        logger.info(f"Discovered {len(scored_results)} potential sources in "
                   f"{time.time() - start_time:.1f}s")
        
        return scored_results
    
    async def _discover_from_url(self,
                                session: aiohttp.ClientSession,
                                url: str,
                                topic_keywords: Set[str],
                                region: Optional[str]) -> List[DiscoveryResult]:
        ...
        try:
            ...
            if not results and self.pages_fetched < self.max_pages - 10:
                sitemap_feeds = await self._discover_from_sitemap(session, url, topic_keywords)
    ...
    async def _autodiscover_feeds(self,
                                 session: aiohttp.ClientSession,
    ...
    async def _check_feed_exists(self,
                                session: aiohttp.ClientSession,
                                url: str) -> bool:
        ...
        try:
            async with session.head(url) as response:
                if response.status == 200:
                    ...
                    return any(t in content_type.lower() for t in ['xml', 'rss', 'atom'])
        except:
            pass
        
        return False
    
    async def _validate_feed(self,
                           session: aiohttp.ClientSession,
                           feed_url: str,
                           topic_keywords: Set[str]) -> bool:
    ...
    async def _discover_from_sitemap(self,
                                    session: aiohttp.ClientSession,
                                    base_url: str,
                                    topic_keywords: Set[str]) -> List[DiscoveryResult]:
                ...
                async with session.get(sitemap_url) as response:
                    ...
                    for url_elem in root.findall('.//{http://www.sitemaps.org/schemas/sitemap/0.9}url'):
                        ...
                        if loc is not None and loc.text:
                            ...
                            if any(term in url_text for term in ['news', 'article', 'blog', 'press']):
                                # Check for topic relevance
                                if any(keyword.lower() in url_text for keyword in topic_keywords):
                                    results.append(DiscoveryResult(
                                        url=loc.text,
                                        domain=domain,
                                        feed_type="sitemap",
    ...
    def _extract_keywords(self, topic: str) -> Set[str]:
        ...
        stopwords = {
            'the', 'in', 'of', 'and', 'or', 'for', 'with', 'on', 'at',
            'to', 'from', 'by', 'about', 'into', 'through', 'during',
    ...
    async def add_to_catalog(self, results: List[DiscoveryResult], region: str):
        ...
        for result in results:
            source = SourceRecord(
                ...
                rss_endpoints=[result.url] if result.feed_type in ["rss", "atom"] else [],
