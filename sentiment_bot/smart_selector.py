...
class SmartSelector:
    ...
    def score_source(
        ...
        topic_score = (
            self._score_topic_relevance(domain, source_topics, topic) if topic else 0.5
        )

        # Regional relevance scoring
        regional_score = (
            self._score_regional_relevance(domain, source_region, region)
            if region
            else 0.5
        )
    ...
    def _score_topic_relevance(
        self, domain: str, source_topics: List[str], target_topic: str
    ) -> float:
