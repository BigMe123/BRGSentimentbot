...
class TopicAnalyzer:
    ...
    def is_about(self, text: str, topic: str) -> Tuple[bool, float]:
        ...
        templates = self.topic_templates.get(topic.lower(), [f"This article is about {topic}"])
    ...
    def extract_topic_tags(self, text: str, topic: str, region: Optional[str] = None) -> List[str]:
    ...
    def _get_tag_candidates(self, topic: str, region: Optional[str] = None) -> List[str]:
        """Get candidate tags for a topic/region combination."""
        
        # Base tags by topic
        topic_tags = {
            'elections': [
                'vote-buying', 'campaign-finance', 'voter-turnout',
                'election-fraud', 'poll-monitoring', 'coalition-building',
                'electoral-reform', 'voting-rights', 'exit-polls'
            ],
            'economy': [
                'inflation', 'unemployment', 'gdp-growth', 'trade-deficit',
                'monetary-policy', 'fiscal-stimulus', 'market-crash',
                'currency-crisis', 'investment', 'sanctions'
            ],
            'security': [
                'terrorism', 'military-action', 'border-conflict',
                'cybersecurity', 'defense-spending', 'peacekeeping',
                'arms-trade', 'insurgency', 'maritime-security'
            ],
            'climate': [
                'carbon-emissions', 'renewable-energy', 'deforestation',
                'extreme-weather', 'paris-agreement', 'green-tech',
                'fossil-fuels', 'sustainability', 'climate-adaptation'
            ]
        }
