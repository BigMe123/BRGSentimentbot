...
class ContentExtractor:
    ...
    POSITIVE_PATTERNS = re.compile(
        r'(article|content|main|body|post|entry|text|story|paragraph)',
        re.IGNORECASE
    )
    ...
    def extract_metadata(soup: BeautifulSoup) -> Dict[str, Any]:
        ...
        for prop in ['article:published_time', 'datePublished', 'publish_date']:
            date_meta = soup.find('meta', {'property': prop}) or soup.find('meta', {'name': prop})
            if date_meta:
    ...
    def smart_extract(cls, soup: BeautifulSoup) -> str:
        ...
        if not main_content:
            ...
            for div in soup.find_all('div', class_=True):
                ...
                if cls.POSITIVE_PATTERNS.search(classes) and not cls.NEGATIVE_PATTERNS.search(classes):
        ...
        text_elements = main_content.find_all(['p', 'div', 'section', 'article', 
                                               'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
                                               'blockquote', 'li', 'td', 'th', 'span'])
...
async def gather_rss(feeds: Iterable[str] | None = None) -> List[ArticleData]:
    ...
    confidence = (volume_score * 0.3 + diversity_score * 0.2 + 
                 success_rate * 0.2 + word_volume_score * 0.3)
