...
class SourceRecord:
    ...
    discovery_method: Optional[str] = None  # "manual", "rss_autodiscovery", "sitemap", "crawl"
...
class SKBCatalog:
    ...
    def _init_db(self):
        ...
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_priority ON sources(priority DESC)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_validation ON sources(validation_status)")
    ...
    def import_from_yaml(self, yaml_path: str):
        ...
        try:
            ...
            self.conn.commit()
            logger.info(f"Successfully imported {self.conn.execute('SELECT COUNT(*) FROM sources').fetchone()[0]} sources")
    ...
    def get_sources_by_region(self, region: str, 
                             limit: Optional[int] = None) -> List[SourceRecord]:
        ...
        sources = [SourceRecord.from_dict(json.loads(row['data'])) 
                  for row in cursor.fetchall()]
        
        self._update_cache(cache_key, sources)
        return sources
    
    def get_sources_by_topic(self, topic: str, 
                            limit: Optional[int] = None) -> List[SourceRecord]:
        ...
        if topics:
            ...
            for topic in topics:
                cursor = self.conn.execute(
                    "SELECT domain FROM topic_index WHERE topic = ?",
                    (topic.lower(),)
                )
    ...
    def fuzzy_match_topics(self, query: str, threshold: float = 0.6) -> List[str]:
        ...
        # Find close matches
        matches = get_close_matches(query_normalized, all_topics, 
                                   n=10, cutoff=threshold)
        
        # Also check for substring matches
        substring_matches = [t for t in all_topics 
                           if query_normalized in t or t in query_normalized]
        
        # Combine and deduplicate
        combined = list(set(matches + substring_matches))
        return combined[:10]  # Return top 10 matches
    
    def update_source_stats(self, domain: str, 
        ...
        self.conn.execute("""
            ...
            WHERE domain = ?
        """, (
            json.dumps(data), historical_yield, reliability_score, 
            freshness_score, error_count, domain
        ))
        self.conn.commit()
    ...
    def add_discovered_source(self, source: SourceRecord):
        ...
        try:
            ...
            """, (
                source.domain, source.name, source.region, source.country,
                json.dumps(source.to_dict()), source.priority,
                source.policy, source.validation_status
            ))
