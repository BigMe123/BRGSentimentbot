...
class ProfessionalMetrics:
    ...
    def _calculate_metrics(
        self, results: Dict, articles: List[Dict], performance: Dict
    ) -> Dict:
        """Calculate comprehensive metrics."""

        # Quality metrics
        sentiments = [
            a.get("sentiment_score", 0) for a in articles if "sentiment_score" in a
        ]
        ...
        positive_aspects = [
            a for a in all_aspects if a.get("sentiment_label") == "positive"
        ]
        negative_aspects = [
            a for a in all_aspects if a.get("sentiment_label") == "negative"
        ]
        ...
        metrics = {
            ...
            "deduplication_rate": (
                1 - (unique_clusters / len(articles)) if articles else 0
            ),
            # Aspects
            ...
            / performance.get("processing_time", 1),
        }
    ...
    def _calculate_diversity(
        self, domains: set, languages: set, countries: set
    ) -> float:
    ...
    def _top_aspects(self, aspects: List[Dict], limit: int = 5) -> List[Dict]:
        ...
        sorted_aspects = sorted(
            aspects, key=lambda a: a.get("importance", 0), reverse=True
        )
