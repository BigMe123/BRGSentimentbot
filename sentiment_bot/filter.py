...
def calculate_relevance_score(
    text: str, title: str, region_keywords: List[str], topic_keywords: List[str]
...
def is_relevant(
    ...
    scores = {"region": region_score, "topic": topic_score, "combined": combined_score}

    # 4. Check thresholds
    if region_score < min_region_score:
        return (
            False,
            f"Low region relevance ({region_score:.2f} < {min_region_score})",
            scores,
        )
    ...
    if topic_score < min_topic_score:
        return (
            False,
            f"Low topic relevance ({topic_score:.2f} < {min_topic_score})",
            scores,
        )
    ...
    if combined_score < min_combined_score:
        return (
            False,
            f"Low combined relevance ({combined_score:.2f} < {min_combined_score})",
            scores,
        )
