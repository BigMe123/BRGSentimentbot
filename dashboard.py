...
**Relevance Screening:** The scan uses a two-pass relevance gate. Before full-text scraping, a permissive prefilter keeps articles with title/body evidence for the requested topic or region so the candidate pool does not collapse too early. After full text is retrieved, a stricter pass requires title-level evidence plus supporting body context. Curated taxonomies cover ambiguous topic families such as energy markets, strategic technology, trade controls, and culture-policy debates; arbitrary user topics fall back to the same generic evidence scorer. Optional Semantic and Model Review modes add embedding or OpenAI checks after the deterministic gate.
