...
for region, sources in RSS_REGISTRY.items():
    ...
    for domain, info in sources.items():
        ...
        if existing:
            ...
            name = domain.replace("www.", "").replace(".com", "").replace(".co.uk", "").replace(".org", "").title()
...
for domain, info in RSS_REGISTRY.get("global", {}).items():
    ...
    if not cursor.fetchone():
        ...
        data = {
            "rss_endpoints": info.get("rss_endpoints", []),
            "topics": ["general", "politics", "economy", "elections", "security", "climate", "tech"],
        ...
        name = domain.replace("www.", "").replace(".com", "").replace(".org", "").title()
