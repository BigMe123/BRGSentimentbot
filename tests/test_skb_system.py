        ...
        with tempfile.NamedTemporaryFile(suffix='.db', delete=False) as tf:
            ...
            for i in range(100):
                source = SourceRecord(
                    ...
                    topics=["tech", "economy"] if i % 2 == 0 else ["politics", "security"],
                    languages=["en"],
                    priority=0.5 + (i / 200),
                    policy="allow"
                )
...
class TestSelectionPlanner:
    ...
    def planner(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.db', delete=False) as tf:
            ...
            topics = ["elections", "security", "economy", "politics", "energy", "climate", "tech"]
    ...
    def test_obscure_topic_handling(self, planner):
        ...
        plan = planner.plan_selection(
            other_topic="semiconductors in Maghreb",
            quotas=quotas
        )
    ...
    def test_time_budget_allocation(self, planner):
        ...
        assert all(1 <= t <= 10 for t in plan.time_allocations.values() 
...
class TestHealthMonitor:
    ...
    def test_metrics_recording(self, monitor):
        ...
        monitor.record_fetch_result("test.com", success=False, latency_ms=10000, 
                                   error_type="timeout")
    ...
    def test_auto_tuning(self, monitor):
        ...
        for _ in range(10):
            monitor.record_fetch_result("bad.com", success=False, latency_ms=10000,
                                       error_type="timeout")
    ...
    def test_run_metrics_aggregation(self, monitor):
        ...
        for i in range(5):
            domain = f"test{i}.com"
            monitor.record_fetch_result(domain, success=i % 2 == 0, latency_ms=1000 * (i + 1))
...
class TestRelevanceFilter:
    ...
    def test_topic_verification(self, filter):
        ...
        score = filter.verify_relevance(
            article=article,
            target_topics=['energy', 'climate']
        )
        
        assert score.topic_score > 0.5
        assert any('solar' in s.lower() or 'energy' in s.lower() 
                  for s in score.topic_signals)
...
class TestAcceptanceCriteria:
    ...
    def test_cold_start_selection_performance(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.db', delete=False) as tf:
            ...
            assert selection_time < 300, f"Cold start took {selection_time:.0f}ms, expected <300ms"
    ...
    def test_warm_start_selection_performance(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.db', delete=False) as tf:
            ...
            assert selection_time < 50, f"Warm start took {selection_time:.0f}ms, expected <50ms"
    ...
    def test_headless_usage_cap(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.db', delete=False) as tf:
            ...
            assert headless_ratio <= 0.10, f"Headless ratio {headless_ratio:.2%} exceeds 10%"
